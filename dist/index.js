...
server.tool('tcu_buscar', 'Busca acórdãos do TCU por palavras-chave. Sincroniza o cache local automaticamente na primeira vez ou quando expirado (>24h). Use para consultar jurisprudência do Tribunal de Contas da União.', {
    texto: z.string().describe('Termos de busca separados por espaço (AND lógico). Ex: "pregão eletrônico TI"'),
    campos: z
    ...
    try {
        ...
        const linhas = resultados.map((a, i) => `## ${i + 1}. Acórdão ${a.numeroAcordao}/${a.anoAcordao} — ${a.colegiado}\n` +
        ...
        const header = `Encontrados **${resultados.length}** acórdão(s) de ${cache.acordaos.length} no cache` +
